...
describe("Webhooks", () => {
  ...
  describe("generateSignedHeader", () => {
    ...
    it('should correctly generate a webhook header with timestamp and hash', () => {
      const header = Sign.generateSignedHeader(MockPAYLOADString, MockSECRET, MockTIMESTAMP);
  ...
  describe("verifyWebhookEvent", () => {
    it("should throw error for invalid json payload", async () => {
      await expect(verifyWebhookEvent("invalid payload", MockSIG, MockSECRET)).rejects.toThrow(/Error in parsing payload/)
    })

    it('should throw error if header or secret is not given', async () => {
      await expect(verifyWebhookEvent(MockPAYLOADString)).rejects.toThrow(/No data to verify/)
    });

    it('should return true if signature match with expected', async () => {   
      const header = Sign.generateSignedHeader(MockPAYLOADString, MockSECRET, MockTIMESTAMP);
      await expect(verifyWebhookEvent(MockPAYLOADString, header, MockSECRET)).resolves.toBeTruthy()
    });
    it('should throw error if signature is verified but time is expired', async () => {   
      const header = Sign.generateSignedHeader(MockPAYLOADString, MockSECRET, MockEXPIREDTIME); 
      await expect(verifyWebhookEvent(MockPAYLOADString, header, MockSECRET)).rejects.toThrow(/The validation time has been expired/)
    });
  });
})